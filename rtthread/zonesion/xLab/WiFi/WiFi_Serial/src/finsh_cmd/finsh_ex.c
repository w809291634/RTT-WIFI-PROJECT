...
extern uint8_t wifi_param_error;
static void wifi_ctrl(int argc, char **argv) {
  extern t_rf_info rf_info;      //通讯模块信息
  if(argc == 1) {
    rt_kprintf("Wi-Fi info:\n");
    rt_kprintf("\tMAC: %s\n", rf_info.mac);
    rt_kprintf("\tSSID: %s\n", rf_info.wifi_ssid);
    rt_kprintf("\tKEY: %s\n", rf_info.wifi_key);
    rt_kprintf("\tAID: %s\n", rf_info.zhiyun_id);
    ...
    if(!rt_strcmp(argv[1], "aid")) {
      ...
      rt_kprintf("Wi-Fi aid set: %s\n", rf_info.zhiyun_id);
      wifi_param_error = 0;
    } else if(!rt_strcmp(argv[1], "akey")) {
      strcpy(rf_info.zhiyun_key, argv[2]);
      rt_kprintf("Wi-Fi akey set: %s\n", rf_info.zhiyun_key);
      wifi_param_error = 0;
    } else if(!rt_strcmp(argv[1], "ssid")) {
      strcpy(rf_info.wifi_ssid, argv[2]);
      rt_kprintf("Wi-Fi ssid set: %s\n", rf_info.wifi_ssid);
      wifi_param_error = 0;
    } else if(!rt_strcmp(argv[1], "key")) {
      if(strcmp(argv[2], "null") == 0) {
        rf_info.wifi_key[0] = 0;
      } else {
        strcpy(rf_info.wifi_key, argv[2]);
      }
      rt_kprintf("Wi-Fi key set: %s\n", rf_info.wifi_key);
      wifi_param_error = 0;
    }
  } else if(argc == 2) {
    if(!rt_strcmp(argv[1], "save")) {
      flash_write(FLASH_PARAM_START_ADDR, (uint16_t *)(&rf_info), sizeof(t_rf_info) / 2);
      rt_kprintf("Wi-Fi config save successed!\n");
    }
  }
}
...
int rf(int argc, char **argv) {
  ...
  char rf_device[3][10] = {"NULL", "CAT1", "Wi-Fi"};

...
static t_sensor_value sensor_value;      //传感器原始数据结构体
...
void sensor_init_a(void) {
  ...
  /***************初始化各传感器***************/
  sensor_fun_p.sensorControl(sensor_value.D1);   //开机恢复此前控制状态
...
static void updateD1(uint8_t type, uint8_t value) {
  ...
  if(sensor_value.D1 != temp) {
    ...
    sensor_fun_p.sensorControl(sensor_value.D1);//执行D1相关控制操作
  }
}
